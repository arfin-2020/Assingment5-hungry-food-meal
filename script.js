...
searchBtn.addEventListener("click", function() {
    ...
    if (search_keyword === '') {
        ...
        getFood(search_keyword);
        warning.style.display = 'none';
    }

});

const displayAllDetails = name => {
...
function getFood(mealId) {
    //const mainApi = `https://www.themealdb.com/api/json/v1/1/search.php?s=${search_keyword.value}`;
    ...
    const displayFoods = foods => {
        ...
        if (foods != null) {
            foods.map(food => {
                ...
                const foodInfo = `
                        <div onclick="displayAllDetails('${food.idMeal}')" class="border rounded text-center h-100" data-bs-toggle="modal" data-bs-target="#exampleModal">
